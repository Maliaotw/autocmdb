...
__all__ = ["Host", "CPU", "NIC", "Disk", "Memory", "Node", "HostRecord", 'idrac','CmdRecord','RunUser','Process',"HostProc"]
...
class idrac(models.Model):
    ...
    port = models.PositiveSmallIntegerField(verbose_name='端口', default=22)
    user = models.CharField(verbose_name='使用者', max_length=64, default=settings.IDRAC_USER)
    passwd = models.CharField(verbose_name='密碼', max_length=64, default=settings.IDRAC_PASSWD)
    host = models.OneToOneField("Host", verbose_name='Host', blank=True, null=True, on_delete=models.CASCADE)
    task = models.ForeignKey("django_celery_beat.PeriodicTask", verbose_name="任務", blank=True, null=True,on_delete=models.CASCADE)
...
class BusinessUnit(models.Model):
    ...
    """
    name = models.CharField('業務線', max_length=64)
    en = models.CharField('英文', max_length=64)
...
class Host(models.Model):
    ...
    asset = GenericRelation(to='asset.Asset')
